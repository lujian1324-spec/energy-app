...
1. **`/device/upgrade/create` request body** — capture one real upgrade started from the console
   on a test unit (the only write this needs), plus its `…/details` and `…/logs` while it runs.
