...
- Fail twice consecutively: confirm a scrollable troubleshooting panel offers power cycling, pairing LED mode, Bluetooth / Nearby devices permission, Location on older Android, moving closer, and QR fallback. Verify Search Again and Scan QR Code work.
...
- Cancel a scan by selecting a device, opening QR, or leaving setup: late callbacks must not add devices, errors, or failures.
