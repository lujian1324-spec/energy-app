...
- **单元 + 接口契约**(`src/**/*.test.ts`,Vitest,`npm run test:unit`,**84 个文件,812 项**,v4.17.3 时;中继 `npm run test:server` 68 项):
...
- **e2e**(Playwright,`tests/`,CI 对本次提交的构建跑,共 50 项;真账号组 5 项另计,仅手动带账号时跑):
