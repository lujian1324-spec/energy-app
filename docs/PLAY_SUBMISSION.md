...
### Privacy policy
- URL:`https://www.sierro.us/pages/policy`（v4.1.2 起 Privacy Policy 迁到营销站，App 内不再有
  `/#/privacy` 路由——旧的 GitHub Pages 链接已失效，提交 Play Console 时务必用这个新地址）
...
> **首次之后可自动化**:完成本节的**首次手动上传**后,后续版本可由 CI 自动发布——
> 配好 Play 服务账号(见 `RELEASE_SIGNING.md` 的「自动发布到 Google Play」),打一个
> `vX.Y.Z` tag 即自动构建签名 AAB 并发布到 closed 轨道(`status=completed`,上传即生效),
> 不必再手动上传 `.aab`。

## 5. 上生产前还需
- 完成上面所有 App content 表单(Play 会亮红点提示未完成项)
- 生产轨道需更完整的商店素材与本地化;内部测试可先跳过精修。
- 自动发布默认走 closed 轨道;要改轨道设仓库变量 `PLAY_TRACK`,生产建议先 `draft` 人工把关。
