...
export default function DevicePage() {
  ...
  const [realtimeCache, setRealtimeCache] = useState<DeviceRealtimeCache>(() => {
    ...
    for (const d of store.devices) {
      ...
      if (state) {
        seed[String(d.id)] = { fields: state.fields, raw: mapFieldsToRealtime(state.fields), loading: false, lastUpdated: Date.now() }
      }
    }
    return seed
  })
  ...
  const fetchDeviceRealtime = useCallback(async (deviceId: number | string) => {
    ...
    if (useDeviceStore.getState().isDemoMode) {
      ...
      if (state) {
        setRealtimeCache(prev => ({ ...prev, [idStr]: { fields: state.fields, raw: mapFieldsToRealtime(state.fields), firingAlarms: state.firingAlarms ?? [], loading: false, lastUpdated: Date.now() } }))
      }
      return
    }
    setRealtimeCache(prev => ({ ...prev, [idStr]: { ...prev[idStr], loading: true } }))
    try {
      const result = await fetchDeviceState(idStr)
      if ((result.code === 0 || result.code === '0') && result.data) {
        setRealtimeCache(prev => ({ ...prev, [idStr]: { fields: result.data!.fields, raw: mapFieldsToRealtime(result.data!.fields), firingAlarms: result.data!.firingAlarms ?? [], loading: false, lastUpdated: Date.now() } }))
      } else {
  ...
  useEffect(() => {
    if (selectedDeviceState && selectedDeviceState.deviceId) {
      ...
      setRealtimeCache(prev => ({ ...prev, [idStr]: { fields: selectedDeviceState.fields, raw: mapped, firingAlarms: selectedDeviceState.firingAlarms ?? [], loading: false, lastUpdated: Date.now() } }))
    }
  }, [selectedDeviceState])
  ...
  const fetchBatteryPassthrough = useCallback(async (deviceId: string | number) => {
    ...
    try {
      ...
      setRealtimeCache(prev => {
        ...
        return { ...prev, [idStr]: { fields: existing?.fields ?? {}, raw: { ...existing?.raw, remainingBatteryCapacity: live.soc, batteryPower: live.batteryPower, acPower: live.acPower, solarPower: live.solarPower, outputPower: live.outputPower, batteryTemp: live.batteryTemp }, loading: false, lastUpdated: Date.now() } }
      })
    } catch {}
  }, [])
  ...
  const togglePower = async (deviceId: string | number, e: React.MouseEvent) => {
    ...
    try {
      const res = await passthroughDevice(idStr, { data: next ? FRAMES.AC_POWER_ON : FRAMES.AC_POWER_OFF, noOutput: true })
      if (!isApiSuccess(res.code)) throw new Error(res.message ?? res.msg ?? 'Power command rejected')
    } catch (err) {
  ...
  useEffect(() => {
    ...
    loadRatedParams(lid).then(p => setLowBatteryCapacityWh(p ? p.acInvOutputPower * 2 : undefined)).catch(() => setLowBatteryCapacityWh(undefined))
  }, [lowBatteryDevice?.id])
  const lowBatteryTimeStr = lowBatteryDevice ? batteryTimeLabel({ acPower: getDeviceNum(lowBatteryDevice.id, 'acPower') ?? 0, solarPower: getDeviceNum(lowBatteryDevice.id, 'solarPower') ?? 0, outputPower: getDeviceNum(lowBatteryDevice.id, 'outputPower') ?? 0, soc: getDeviceNum(lowBatteryDevice.id, 'remainingBatteryCapacity') ?? 0, capacityWh: lowBatteryCapacityWh, isCharging: (getDeviceNum(lowBatteryDevice.id, 'batteryPower') ?? 0) > 0 }) : '--'
  ...
  const BatteryTag = ({ level, connected, charging, unknown }: { level: number; connected: boolean; charging: boolean; unknown?: boolean }) => {
    if (!connected) return (<span className="inline-flex items-center px-3 py-1.5 rounded-full bg-[#4B1512] text-danger text-body-md font-semibold">Disconnected</span>)
    if (unknown) return (<span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-ink-9"><span className="w-[22px] h-[12px] rounded-s border-s animate-pulse" style={{ borderColor: '#8C8C8C' }} /><span className="text-body-md font-semibold text-ink-7">--%</span></span>)
  ...
  const PowerToggle = ({ deviceId, on, disabled }: { deviceId: string | number; on: boolean; disabled: boolean }) => (
    <button onClick={(e) => { if (!disabled) togglePower(deviceId, e) }} disabled={disabled} aria-label="Power toggle" className={`relative w-[52px] h-[31px] rounded-full transition-colors duration-200 flex-shrink-0 ${disabled ? 'bg-ink-9 opacity-50 cursor-not-allowed' : on ? 'bg-primary active:scale-95' : 'bg-ink-9 active:scale-95'} transition-transform`}>
      <span className={`absolute top-[2px] w-[27px] h-[27px] rounded-full bg-white shadow-sm transition-[left,transform] duration-200 ${on && !disabled ? 'left-[23px]' : 'left-[2px]'}`} />
    </button>
  )
      ...
      <div className="h-full flex flex-col bg-ink-12 overflow-hidden">
        <div className="px-5 pt-4 pb-3 safe-area-top">
          <div className="flex justify-between items-center h-11 mb-3">
            <h1 className="text-display font-display text-white leading-none">Device</h1>
          </div>
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-ink-12 overflow-hidden">
      ...
      <DevicePageOverlays showAddModal={showAddModal} setShowAddModal={setShowAddModal} navigate={navigate} realtimeCache={realtimeCache} fetchDeviceRealtime={fetchDeviceRealtime} getDeviceNum={getDeviceNum} getDeviceField={getDeviceField} getWorkModeLabel={getWorkModeLabel} getDeviceImage={getDeviceImage} />
    </div>
  )
}
