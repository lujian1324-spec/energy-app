  ...
  return (
    <div className="h-full flex flex-col bg-[#080E1A] overflow-hidden">
      ...
      <div className="px-5 pt-4 pb-4 safe-area-top">
        <h2 className="text-xl font-bold text-[#E8F4FF]">Output Control</h2>
        <p className="text-xs text-[#7A9AB8] mt-1">Manage Charge / Discharge · Live Power Monitor</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-5 pb-4">
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-[rgba(0,212,255,0.06)] to-[rgba(0,255,156,0.04)]
                     border border-[rgba(0,212,255,0.2)] rounded-[28px] p-5 mb-4 relative overflow-hidden"
        >
          {/* 背景光晕 */}
          <div className="absolute -bottom-8 -right-8 w-[120px] h-[120px] rounded-full 
                        bg-[radial-gradient(circle,rgba(0,255,156,0.08),transparent_70%)]" />
          
          {/* 三列功率显示 */}
          <div className="flex justify-between mb-4">
            <div className="text-center flex-1">
              <div className="flex items-center justify-center gap-1.5">
                <Sun size={18} className="text-[#00FF9C]" />
                <span className="text-[22px] font-extrabold text-[#00FF9C] tracking-tight">
                  +{powerStation.inputPower}<small className="text-[13px] font-normal text-[#00CC7A]">W</small>
                </span>
              </div>
              <div className="text-[10px] text-[#7A9AB8] mt-1 tracking-wide uppercase">Solar In</div>
            </div>
            <div className="w-px bg-[rgba(0,212,255,0.08)] my-1" />
            <div className="text-center flex-1">
              <div className="flex items-center justify-center gap-1.5">
                <Zap size={18} className="text-[#00D4FF]" />
                <span className="text-[22px] font-extrabold text-[#00D4FF] tracking-tight">
                  -{powerStation.outputPower}<small className="text-[13px] font-normal text-[#00AACC]">W</small>
                </span>
              </div>
              <div className="text-[10px] text-[#7A9AB8] mt-1 tracking-wide uppercase">Output</div>
            </div>
            <div className="w-px bg-[rgba(0,212,255,0.08)] my-1" />
            <div className="text-center flex-1">
              <div className="flex items-center justify-center gap-1.5">
                <Battery size={18} className={powerStation.inputPower >= powerStation.outputPower ? 'text-[#FFB800]' : 'text-[#FF4757]'} />
                {(() => {
                  const net = powerStation.inputPower - powerStation.outputPower
                  const color = net >= 0 ? '#FFB800' : '#FF4757'
              ...
              <div className="text-[10px] text-[#7A9AB8] mt-1 tracking-wide uppercase">Net Charge</div>
            </div>
          </div>
            ...
            <svg className="w-full h-full" viewBox="0 0 750 48" preserveAspectRatio="none">
              ...
              <g className="animate-wave">
                ...
                <path d="M0,28 C30,14 60,42 90,28 C120,14 150,38 180,24 C210,10 240,36 270,22 C300,8 330,34 360,20 C390,6 420,32 450,18 C480,4 510,30 540,16 C570,2 600,28 630,14 C660,0 690,26 720,12" 
                      fill="none" stroke="#00FF9C" strokeWidth="1.5" opacity="0.7"/>
              </g>
              {/* 输出波形 */}
            ...
            {outputPorts.map((port) => (
              ...
              >
                <div className={`
                  ...
                  ${port.type === 'ac-out' ? 'bg-[rgba(0,212,255,0.1)] text-[#00D4FF]' : ''}
                  ${port.type === 'usb-out' ? 'bg-[rgba(255,184,0,0.1)] text-[#FFB800]' : ''}
                `}>
                  {port.type === 'ac-out' ? <Zap size={20} /> : <Battery size={20} />}
                </div>
                <div className="flex-1">
                  <div className="text-sm font-semibold text-[#E8F4FF]">
                    {port.name}
                  </div>
                ...
                <div className="flex flex-col items-end gap-1.5">
                  <div className={`text-[15px] font-bold ${port.status === 'active' ? 'text-[#00D4FF]' : 'text-[#3D5A78]'}`}>
        ...
        <div className="bg-[#111E33] border border-[rgba(0,212,255,0.08)] rounded-[20px] p-4 mb-4">
          <div className="flex items-center gap-2 text-sm font-bold text-[#E8F4FF] mb-3.5">
            <Settings size={14} className="flex-shrink-0" />
            CHARGE PROTECTION
          </div>
          
          <div className="space-y-3">
            <div className="flex justify-between items-center py-2 border-b border-[rgba(0,212,255,0.08)]">
              <div>
                <div className="text-[13px] text-[#9BB8D4]">Charge Mode</div>
                <div className="text-[10px] text-[#3D5A78] mt-0.5">Current: Standard</div>
              </div>
              <div className="text-[13px] font-semibold text-[#00D4FF]">Standard ▾</div>
            </div>
            
            <div className="flex justify-between items-center py-2 border-b border-[rgba(0,212,255,0.08)]">
              <div>
                <div className="text-[13px] text-[#9BB8D4]">Over-temp Protection</div>
                <div className="text-[10px] text-[#3D5A78] mt-0.5">Auto cut-off above 45°C</div>
              </div>
              <ToggleSwitch 
            ...
            <div className="flex justify-between items-center py-2 border-b border-[rgba(0,212,255,0.08)]">
              <div>
                <div className="text-[13px] text-[#9BB8D4]">Over-discharge Protection</div>
                <div className="text-[10px] text-[#3D5A78] mt-0.5">Keep minimum 5% reserve</div>
              </div>
              <ToggleSwitch 
            ...
            <div className="flex justify-between items-center py-2">
              <div>
                <div className="text-[13px] text-[#9BB8D4]">ECO Mode</div>
                <div className="text-[10px] text-[#3D5A78] mt-0.5">Auto sleep when load &lt;10W</div>
              </div>
              <ToggleSwitch 
          ...
          <div className="mt-4">
            <div className="flex justify-between items-center mb-2.5">
              <span className="text-[13px] text-[#9BB8D4]">Charge Limit</span>
              <span className="text-base font-bold text-[#00FF9C]">{settings.chargeLimit}%</span>
            </div>
            <div className="relative h-5 flex items-center">
              ...
              {/* 已填充部分 */}
              <div
                className="absolute left-0 h-2 rounded-full bg-gradient-to-r from-[#00CC7A] to-[#00FF9C]
                           shadow-[0_0_10px_rgba(0,255,156,0.3)] pointer-events-none"
                style={{ width: `${(settings.chargeLimit - 50) / 50 * 100}%` }}
              />
              {/* 拇指指示点：用 calc 避免边界溢出 */}
              <div
                className="absolute w-5 h-5 rounded-full bg-white border-2 border-[#00FF9C]
                           shadow-[0_0_10px_rgba(0,255,156,0.4),0_2px_6px_rgba(0,0,0,0.3)]
                           pointer-events-none"
                style={{
            ...
            <div className="flex justify-between text-[10px] text-[#3D5A78] mt-1.5 px-0.5">
