...
import { crc16modbus, decodeLiveStatus, LIVE_STATUS_MIN_REGISTERS } from '../protocols/modbusProtocol'
...
describe('readLivePassthroughOnce', () => {
  ...
  it('rejects a frame too short to carry SOC, rather than reporting 0%', async () => {
    ...
    passthroughDevice.mockResolvedValue({
      code: 0, data: { base64Output: replyFrame(LIVE_STATUS_MIN_REGISTERS, live) },
    })
