...
describe('BLE provisioning multi-packet reassembly', () => {
  ...
  it('keeps earlier packets when the plugin reuses one ArrayBuffer (iOS indication)', async () => {
    const mgr = await connectManager()
    const packets = makeResponsePackets(WIFI_RESP)
    expect(packets.length).toBeGreaterThan(1)
    const p = mgr.scanAp()
    await tick()
    const scratch = new ArrayBuffer(Math.max(...packets.map(x => x.byteLength)))
    for (const pkt of packets) {
      const view = new Uint8Array(scratch, 0, pkt.byteLength)
      view.set(pkt)
      h.notifyCb!(new DataView(scratch, 0, pkt.byteLength))
    }
    const resp = await p
    expect(resp.RC).toBe(0)
    expect(Array.isArray(resp.PL) && resp.PL).toHaveLength(16)
