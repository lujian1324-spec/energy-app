...
import { decodeLiveStatus, decodePowerU16, INVALID_POWER_U16 } from './modbusProtocol'
...
describe('decodeLiveStatus 0xFFFF power sentinel', () => {
  it('omits outputPower / acPower / solarPower when raw === 0xFFFF (not 65535)', () => {
    ...
    regs[0x04] = INVALID_POWER_U16 // output
    regs[0x06] = INVALID_POWER_U16 // solar
    regs[0x07] = INVALID_POWER_U16 // ac
    regs[0x1a] = 856
  ...
  it('omits only the invalid power channel and skips batteryPower', () => {
    const regs = new Array(0x24).fill(0)
    regs[0x04] = INVALID_POWER_U16 // output invalid
    regs[0x06] = 120
    regs[0x07] = 400
    const live = decodeLiveStatus(regs)
    expect(live.outputPower).toBeUndefined()
  ...
  it('decodePowerU16 maps 0xFFFF → undefined and other values through', () => {
