...
describe('saveSmartSchedule — the device is not there (AC-13-1 / AC-13-2)', () => {
  it('fails visibly if the offline save cannot be persisted', async () => {
    vi.spyOn(localStorage, 'setItem').mockImplementation(() => { throw new Error('quota') })
    const r = await saveSmartSchedule(A, win(), { connectivity: DEVICE_OFFLINE })
    expect(r.ok).toBe(false)
    expect(r.queued).toBe(false)
    expect(r.applied?.detail).toContain('store')
  })

  it('does not queue without an account identity', async () => {
    store.delete('iot_user_id')
    const r = await saveSmartSchedule(A, win(), { connectivity: DEVICE_OFFLINE })
    expect(r.ok).toBe(false)
  })

  it('does not queue an old Smart disable over active Sleep Mode', async () => {
    setActiveScheduleMode(A, 'sleep')
    const r = await saveSmartSchedule(A, win({ enabled: false }), { connectivity: DEVICE_OFFLINE })
    expect(r.ok).toBe(false)
...
describe('flushPendingSmartSchedule (AC-13-4 / AC-13-5)', () => {
  ...
  it('gives up after a bounded number of refusals rather than retrying forever', async () => {
    ...
    await flushPendingSmartSchedule(A, { connectivity: ONLINE, apply })
    expect(apply).toHaveBeenCalledTimes(MAX_FLUSH_ATTEMPTS)
  })
...
describe('flush keeps the active mode in step (SW-12, AC-13-8)', () => {
  it('cancels an old queued Smart save after a successful Sleep claim', async () => {
    await saveSmartSchedule(A, win(), { connectivity: DEVICE_OFFLINE })
    setActiveScheduleMode(A, 'sleep')
    const { apply } = stubApply(ok())
    expect((await flushPendingSmartSchedule(A, { connectivity: ONLINE, apply })).status).toBe('none')
    expect(apply).not.toHaveBeenCalled()
    expect(getActiveScheduleMode(A)).toBe('sleep')
  })

  it('does not charge an old refusal against a newer save', async () => {
    enqueueSmartScheduleSave(A, win(), 1000)
    const apply = vi.fn(async () => {
      enqueueSmartScheduleSave(A, win({ chargePowerW: 200 }), 1000)
      return rejected()
    })
    const r = await flushPendingSmartSchedule(A, { connectivity: ONLINE, apply })
    expect(r.status).toBe('none')
  ...
  it('does not clear or claim after an account change during replay', async () => {
    enqueueSmartScheduleSave(A, win(), 1000)
    const apply = vi.fn(async () => {
    ...
    const r = await flushPendingSmartSchedule(A, { connectivity: ONLINE, apply })
    expect(r.status).toBe('none')
    expect(getPendingSmartScheduleSave(A)?.window.chargePowerW).toBe(200)
...
describe('a flush is the same A→B→C run a live save makes (AC-13-5 / AC-13-9)', () => {
  ...
  it('checks the account again between config and power writes', async () => {
    enqueueSmartScheduleSave(A, win(), 1000)
    vi.spyOn(h.api, 'post').mockImplementationOnce(async () => {
      store.set('iot_user_id', 'other-account')
      return { code: 0, data: {} }
    })
    const r = await flushPendingSmartSchedule(A, { connectivity: ONLINE })
    expect(r.status).toBe('none')
    expect(paths().some(p => p.includes('passthrough'))).toBe(false)
  })
