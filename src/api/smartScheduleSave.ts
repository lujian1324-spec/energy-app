...
export async function saveSmartSchedule(
  ...
  if (before) {
    ...
    enqueueSmartScheduleSave(deviceId, window, now())
    return { ok: true, queued: true, offlineReason: before }
  }

  const applied = await apply(deviceId, window)
  if (applied.ok) {
    // The device holds something newer than any pending entry for it.
    clearPendingSmartScheduleSave(deviceId)
  ...
  if (dropped) {
    enqueueSmartScheduleSave(deviceId, window, now())
    return { ok: true, queued: true, offlineReason: dropped, applied }
  }
...
export interface FlushPendingResult {
  ...
  attempts?: number
  /** Was the pending save dropped for exhausting the ladder? */
  gaveUp?: boolean
}
...
export async function flushPendingSmartSchedule(
  ...
  const apply = opts.apply ?? applySmartSchedule
  const applied = await apply(deviceId, pending.window)
  ...
  const { attempts, gaveUp } = recordFlushAttempt(deviceId)
