 ...
 *
 * A and B are reported as failures because the user's Save did not take; C is
 * best-effort (an unconfigured or unreachable relay leaves client-side timing
 * working), so it only sets `relayAccepted`.
 */
export async function applySmartSchedule(
  ...
  try {
    ...
    return { ok: false, failedStep: 'config', detail: errText(e), phase, relayAccepted: false }
  }
  ...
  try {
    ...
    return { ok: false, failedStep: 'passthrough', detail: errText(e), phase, relayAccepted: false }
  }
  ...
  return { ok: true, wattsWritten: watts, phase, relayAccepted }
}
